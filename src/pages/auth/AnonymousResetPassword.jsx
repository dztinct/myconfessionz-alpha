...
const AnonymousResetPassword = () => {
  ...
  const { user, resetPassword, isLoading, error } = useAuthStore()
  ...
  const navigate = useNavigate()
    ...
    const onSubmit = async (data, event) => {
      ...
      try {
        console.log(data)
        await resetPassword(user.username, data.password, data.password_confirmation)
  ...
  return (
    <div className='text-center h-screen py-24 bg-black text-white mx-auto'>
        ...
        <p>Reset Password (Anonymous)</p>

        <form onSubmit={handleSubmit(onSubmit)}>
          
          <input type="password" id="anonymous-reset-password" 
...
export default AnonymousResetPassword