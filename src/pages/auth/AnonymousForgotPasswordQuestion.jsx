...
const AnonymousForgotPasswordQuestion = () => {
        // Define validation schema based on question
        const schema = yup.object().shape({
            answer: yup.string().required("Answer is required"),
          });
        
        const { forgotPasswordQuestion, user, error, isLoading } = useAuthStore()
        ...
        const navigate = useNavigate()
          ...
          const onSubmit = async (data, event) => {
            ...
            try {
              console.log(data)
              await forgotPasswordQuestion(user.username, user.recovery_question, data.answer)
              toast.success("Response Correct");
                navigate('/anonymous-reset-password');
  ...
  return (
    ...
    <div className="flex items-center mb-3">
          ...
          >
            <option value={user.recovery_question}>{user.recovery_question}</option>
          </select>
        </div>
...
export default AnonymousForgotPasswordQuestion
